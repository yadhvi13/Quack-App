...
function Sidebar({selectedUser, setSelectedUser}) {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-auto scrollbar-hide  text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      <div className='pb-5'>

        {/* logo and menu */}
        <div className='flex justify-between items-center'>
            <img src={assets.quacklogo2} alt="logo" className='w-32 h-auto object-contain max-h-20' />
      ...
      <div className='flex flex-col'>
         {userDummyData.map((user,index)=>(
            <div onClick={()=> {setSelectedUser(user)}}
                ...
                <img src={user?.profilePic || assets.avatar_icon} alt="userprofilephoto" className='w-[35px] aspect-[1/1] rounded-full' />
                <div className='flex flex-col leading-5'>
                    <p>{user.fullName}</p>
                    {
                        index < 3 ? <span className='text-green-400 text-xs'>Online</span> : <span className='text-neutral-400 text-sm'>Offline</span>
                    }
                </div>
